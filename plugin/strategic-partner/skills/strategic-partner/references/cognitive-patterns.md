# Cognitive Operations — Advisory Thinking Tools
...
Separate the thing from its history or its surface, examine it on its own, then
decide. Fires when a removal or cleanup is proposed, or when complexity needs
judging.

### Chesterton's Fence (Chesterton)
...
Zoom in to surface hidden work, or zoom out to find the core and cut to it.
Fires when a task is called "small," or when scope is sprawling.
...
Watch whether the reasoning holds, and break in with a failure-mode scan or an
inversion when it doesn't. Fires while a recommendation is forming, or just
...
Sit with the open question or commit — 70% of the facts is enough on a
reversible call, and resist solving everything at once. Fires when the
conversation loops, or a rewrite urge builds.
...
- Exception: one-way doors (see Standalone gates) — get to 90%.
...
Work from a simpler model or return to full detail — separate the prep change
from the behavior change. Fires when a change mixes enabling work with the real
change.
...
Check structural correspondence — does the design fit who will maintain it, and
is a new dependency worth its risk. Fires when an approach adds novelty, or when
architecture meets team boundaries.
...
These three are hard yes/no checks wired to one decision each, not oscillating
thinking-moves. They carry most of the real load in practice.
...
### Blast Radius Instinct (Eng Management)
