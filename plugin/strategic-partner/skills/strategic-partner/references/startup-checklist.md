...
the main `/strategic-partner-plugin:strategic-partner` command (or its `/sp`
and `/advisor` aliases), `/strategic-partner-plugin:status`, and a continuation
that names a handoff note. Steps 1 to 3 always run; Steps 4 and 5 run only
when the live state earns them.
...
- ⚡ Update available (`g6.diff=behind` in the floor results): one line with the version diff and the user's plugin update route
...
### Project-rules drift scan (no cost in the plugin packaging)
...
## 🔧 Reference B: Plugin Registration and Inline Detections (not opening-path work)
...
**Command registration** is handled by Claude Code's plugin loader. Reference B
reads the plugin-native floor signal; there is no runtime setup path.

### Agent E: Removed

**Version check** is handled by the plugin floor sentinel. Reference B names
the fields; the opening never issues its own version request.
