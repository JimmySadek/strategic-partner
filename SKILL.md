...
hooks:
  PreToolUse:
    - matcher: "Agent|Task|Edit|Write|MultiEdit|NotebookEdit|Bash|mcp__serena__|mcp__plugin_serena_serena__"
      hooks:
        - type: command
          command: |
            INPUT=$(cat)
...
initial_instructions (once per session, when exposed)
  └─ get_current_config
       ...
       │     → launcher is misconfigured; route to /strategic-partner:serena
