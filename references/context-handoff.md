...
| Context Level | Action | Behavior |
|---|---|---|
| **70%** | Soft trigger | Begin preparing state summary internally |
| **75%** | Hard trigger | Propose handoff via AskUserQuestion |
| **85%** | Emergency | Execute handoff immediately |

**Check cadence**: after every major deliverable, before starting new analysis, every 3rd exchange.

The cost of an early handoff offer is one AskUserQuestion. The cost of missing it is
losing all session state including unrun implementation prompts.
