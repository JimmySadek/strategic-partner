...
- **Class C — floor signal + model tool call.** `memory=ok` requires the model to call `list_memories` and read `project_overview` plus the most recent `decision_log` entries per `references/startup-checklist.md` Step 4. Findings and backlog may require reading file contents to surface met triggers or urgent items.
