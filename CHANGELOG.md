...
- **Captures auto-archive after surviving three triages without promotion** — a finding written to `.handoffs/findings-MMDD.md` that survives **3 consecutive triage events** (release boundaries plus on-demand triage scans) without being promoted to `.backlog/` is auto-archived: SP appends a note `[auto-archived YYYY-MM-DD — not promoted after 3 triages]` and moves the finding to `.handoffs/findings-archive/`. Reversible (the user can move it back), but the signal is clear — if a finding survived three triages without anyone caring, it probably isn't actionable. Keeps findings files from accumulating indefinitely.
