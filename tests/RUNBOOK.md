...
> Historical: written against the v5.12 pipeline; the behavior under test is now specified by SKILL.md § Decision Ownership Gate. Pass criteria unchanged.
...
## Rule 7 render-before-ask fixtures (v7.0.0)

- `tests/fixtures/v7.0.0/` — two JSONL transcript fixtures plus `run-rule7-fixtures.sh`, measuring the Stop hook's render-before-ask rule against the live SKILL.md hook (false-positive prev-turn reference: MISMATCH — the rule fires; false-negative bare "Approve?": MATCH — the rule stays silent). Measurement only; results recorded 2026-06-11 in `.backlog/add-anti-swallow-render-rule.md` § Codex review follow-up.
